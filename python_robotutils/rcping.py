...
def generate_argparser():
    ...
    sizegroup.add_argument('-l', dest='size', metavar='size', action='store',
                           type=int, help='size of autogenerated body')
    body_help = "send PAYLOAD, which has the form bodytype[::body] or ::body"
    sizegroup.add_argument('-body', dest='payload', help=body_help)
...
def parse_args(args):
    ...
    try:
        ...
        params.port = port
        bodytype, body = parse_body(params.payload)
...
_BAD_BODYTYPE_CHARS = string.whitespace + ','
_BODY_SEPARATOR = '::'
def parse_body(payload):
    """Parse payload, which has the form type or ::body or type::body"""
    bodytype, body = None, None
    if payload:
        bodytype, body = '', ''
        errmsg = '\n'.join(("Invalid body '{}'".format(payload),
                            "Body should have the form TYPE or TYPE::BODY or ::BODY"))
        # We pick the first instance of '::'
        try:
            index = payload.index(_BODY_SEPARATOR)
            bodytype = payload[:index]
            body = payload[index+len(_BODY_SEPARATOR):] # skip past '::'
        except ValueError:
            bodytype = payload
        if bodytype and containschars(bodytype, _BAD_BODYTYPE_CHARS):
            raise ValueError(errmsg)
...
def main(args):
    ...
    try:
        ...
        _LOGGER.exception("run time exception")
    finally:
        client.close()
